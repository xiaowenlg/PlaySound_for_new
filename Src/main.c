...
/*
旧版本存在的bug
在播报过程中运动，当播报快完毕时，又停止运动导致
sportCount = 0  ======>不进入播报函数
同时isCloseSystem变量又被置0，导致不关机
解决办法:在播报完毕后不清零SportCount，用来触发下一次播报
*/
/* Includes ------------------------------------------------------------------*/
#include "main.h"
#include "cmsis_os.h"
...
int main(void)
{
  ...
  MX_ADC1_Init();
  MX_TIM2_Init(1000-1);
  MX_USART_UART_Init(UART_BLE, USART1, 57600);
